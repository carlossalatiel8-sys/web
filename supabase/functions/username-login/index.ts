...
// Inicia sesión por nombre de usuario sin hacer público el correo asociado.
// La respuesta es exactamente la misma para un usuario inexistente o una
// contraseña incorrecta, evitando revelar qué nombres están registrados.
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!/^[a-z0-9_-]{3,20}$/.test(normalizedUsername) || typeof password !== 'string' || password.length < 8) {
